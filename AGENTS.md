...
1. Pick the release version, for example `0.2.0-beta.9.0.9`. If the user asks for a minor update on an active release line, keep the chosen minor version (for example `0.3.0`) and do not bump again to a patch or another new version unless the user explicitly requests a different version.
