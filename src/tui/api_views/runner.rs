...
use super::{dashboard::render_no_flows_hint, diff, environment, truncate};
...
fn render_subtab_bar(f: &mut Frame, app: &ApiApp, area: Rect) {
    ...
    let p = Paragraph::new(Line::from(spans)).style(Style::default().bg(BG_SURFACE));
...
fn render_live_execution(f: &mut Frame, app: &ApiApp, area: Rect) {
    ...
    let items: Vec<ListItem> = steps
        ...
        .map(|(idx, step)| {
            ...
            let line1 = Line::from(vec![
                ...
                Span::styled("  ", Style::default()),
                Span::styled(
                    truncate(&step.url, area.width.saturating_sub(44) as usize),
                ...
                if let Some(body) = &step.request_body {
                    if !body.is_empty() {
                        item_lines.push(Line::from(vec![
                            ...
                            Span::styled(
                                truncate(body, area.width.saturating_sub(18) as usize),
    ...
    if !steps.is_empty() {
        list_state.select(Some(
            app.live_selected_step.min(steps.len().saturating_sub(1)),
        ));
    }
...
fn render_latency_profiler(f: &mut Frame, app: &ApiApp, area: Rect) {
    ...
    let max_ms = run
        ...
        .unwrap_or(1)
        .max(1);
    let bar_width = if inner.width > 30 {
        (inner.width as usize).saturating_sub(32)
    } else {
        20
    };
    ...
    for step in &sorted_steps {
        ...
        lines.push(Line::from(vec![
            Span::styled(
                format!("  {:<20}", truncate(&step.node_id, 20)),
                Style::default().fg(TEXT_DIM),
            ),
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!("{:<8}", format!("{}ms", percentile(&run.steps, 50))),
            Style::default().fg(WHITE).add_modifier(Modifier::BOLD),
        ),
        Span::styled(" P95: ", Style::default().fg(TEXT_DIM)),
        Span::styled(
            format!("{:<8}", format!("{}ms", percentile(&run.steps, 95))),
            Style::default().fg(WHITE).add_modifier(Modifier::BOLD),
        ),
        Span::styled(" P99: ", Style::default().fg(TEXT_DIM)),
        Span::styled(
            format!("{:<8}", format!("{}ms", percentile(&run.steps, 99))),
            Style::default().fg(WHITE).add_modifier(Modifier::BOLD),
        ),
    ]));
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            format!("{:<8}", format!("{}ms", max_ms)),
            Style::default().fg(WHITE).add_modifier(Modifier::BOLD),
        ),
        Span::styled(" Avg: ", Style::default().fg(TEXT_DIM)),
        Span::styled(
            format!("{:<8}", format!("{}ms", avg)),
            Style::default().fg(WHITE).add_modifier(Modifier::BOLD),
        ),
