...
pub fn cmd_diff(package: &str, v1: &str, v2: &str, ecosystem: Option<&str>) {
    ...
    println!(
        ...
        v1.truecolor(255, 100, 100), v2.truecolor(100, 255, 100),
    ...
    let osv_eco = match eco { "pip" => "PyPI", "cargo" => "crates.io", o => o };
    let tuples = vec![
    ...
    if let Ok(results) = osv::batch_query(&tuples) {
        ...
        println!();
        let icon = |n: usize| if n > 0 { "⚠".bright_yellow().to_string() } else { "✔".bright_green().to_string() };
        let count = |n: usize| if n > 0 { n.to_string().bright_red().bold().to_string() } else { "0".bright_green().to_string() };
        println!("  {}  v{}: {} CVE(s)", icon(v1_n), v1, count(v1_n));
        println!("  {}  v{}: {} CVE(s)", icon(v2_n), v2, count(v2_n));
        if v2_n < v1_n {
            println!("\n  {}  {} v{} has fewer vulnerabilities", "→".bright_green(), "Upgrade to".truecolor(100, 200, 100), v2);
        }
    }
    println!();
}
...
fn diff_npm(client: &reqwest::blocking::Client, package: &str, v1: &str, v2: &str) {
    ...
    #[derive(Deserialize)] struct V { dependencies: Option<HashMap<String, String>>, scripts: Option<HashMap<String, String>>, dist: Option<D> }
    #[derive(Deserialize)] struct D { #[serde(rename = "unpackedSize")] unpacked_size: Option<u64> }

    let u1 = format!("https://registry.npmjs.org/{}/{}", registry::urlenc(package), v1);
    let u2 = format!("https://registry.npmjs.org/{}/{}", registry::urlenc(package), v2);
    ...
    let (d1, d2) = match (d1, d2) {
        (Some(a), Some(b)) => (a, b),
        _ => { Logger::error("Could not fetch version metadata from npm."); return; }
    };

    // Size
    if let (Some(s1), Some(s2)) = (d1.dist.as_ref().and_then(|d| d.unpacked_size), d2.dist.as_ref().and_then(|d| d.unpacked_size)) {
        let diff = s2 as i64 - s1 as i64;
        println!();
        println!("  {}  {}", "Size".bold().truecolor(255, 170, 50), "─".repeat(40).truecolor(40, 40, 60));
        println!("     v{}: {}  ·  v{}: {}  ({}{})", v1, format_bytes(s1), v2, format_bytes(s2),
            if diff >= 0 { "+" } else { "" }, format_bytes(diff.unsigned_abs()));
    }

    print_deps_diff("Dependencies", &d1.dependencies.unwrap_or_default(), &d2.dependencies.unwrap_or_default());

    let s1 = d1.scripts.unwrap_or_default();
    let s2 = d2.scripts.unwrap_or_default();
    if s1 != s2 {
        println!();
        println!("  {}  {}", "Scripts".bold().truecolor(255, 170, 50), "─".repeat(38).truecolor(40, 40, 60));
        for k in s2.keys() { if !s1.contains_key(k) { println!("     {} {} (new)", "+".bright_green(), k.bold()); } }
        for k in s1.keys() { if !s2.contains_key(k) { println!("     {} {} (removed)", "-".bright_red(), k.bold()); } }
        for (k, v2) in &s2 { if s1.get(k).map_or(false, |v1| v1 != v2) { println!("     {} {} (changed)", "~".bright_yellow(), k.bold()); } }
    }
}
...
fn diff_pypi(client: &reqwest::blocking::Client, package: &str, v1: &str, v2: &str) {
    ...
    #[derive(Deserialize)] struct I { requires_dist: Option<Vec<String>> }
    ...
    let (d1, d2) = match (d1, d2) {
        (Some(a), Some(b)) => (a, b),
        _ => { Logger::error("Could not fetch metadata from PyPI."); return; }
    };
...
fn diff_crates(client: &reqwest::blocking::Client, package: &str, v1: &str, v2: &str) {
    use serde::Deserialize;
    #[derive(Deserialize)] struct R { version: VI }
    #[derive(Deserialize)] struct VI { crate_size: Option<u64> }
    #[derive(Deserialize)] struct DR { dependencies: Vec<Dep> }
    #[derive(Deserialize)] struct Dep { crate_id: String, req: String, kind: String }
    ...
    if let (Some(r1), Some(r2)) = (d1, d2) {
        if let (Some(s1), Some(s2)) = (r1.version.crate_size, r2.version.crate_size) {
            let diff = s2 as i64 - s1 as i64;
            println!();
            println!("  {}  {}", "Crate Size".bold().truecolor(255, 170, 50), "─".repeat(35).truecolor(40, 40, 60));
            println!("     v{}: {}  ·  v{}: {}  ({}{})", v1, format_bytes(s1), v2, format_bytes(s2),
                if diff >= 0 { "+" } else { "" }, format_bytes(diff.unsigned_abs()));
        }
    }

    let du1 = format!("https://crates.io/api/v1/crates/{}/{}/dependencies", package, v1);
    let du2 = format!("https://crates.io/api/v1/crates/{}/{}/dependencies", package, v2);
    ...
    if let (Some(d1), Some(d2)) = (dd1, dd2) {
        let m = |deps: Vec<Dep>| -> HashMap<String, String> {
            deps.into_iter().filter(|d| d.kind == "normal")
                .map(|d| (d.crate_id, d.req)).collect()
        };
...
fn print_deps_diff(label: &str, old: &HashMap<String, String>, new: &HashMap<String, String>) {
    ...
    let changed: Vec<&String> = new.keys().filter(|k| old.get(*k).map_or(false, |v| v != &new[*k])).collect();
    if added.is_empty() && removed.is_empty() && changed.is_empty() { return; }
    ...
    for n in &added   { println!("     {} {} {}", "+".bright_green(), n.bold(), new[*n].truecolor(100, 255, 100)); }
    for n in &removed { println!("     {} {} {}", "-".bright_red(), n.bold(), old[*n].truecolor(255, 100, 100)); }
    for n in &changed { println!("     {} {} {} → {}", "~".bright_yellow(), n.bold(), old[*n].truecolor(180, 100, 100), new[*n].truecolor(100, 255, 100)); }
}
