...
pub fn cmd_why(package: &str, pkg_file: Option<&str>) {
    ...
    let found: Vec<&scanner::LockedPackage> = packages
        .iter()
    ...
    if !paths.is_empty() {
        ...
        for path in &paths {
            ...
            for (j, step) in path.iter().enumerate() {
                if j > 0 {
                    print!("{}", " → ".truecolor(80, 80, 100));
                }
                if j == 0 {
                    print!("{}", step.bold().truecolor(0, 210, 255));
                } else if j == path.len() - 1 {
                    print!("{}", step.bold().bright_green());
                } else {
                    print!("{}", step.bold().truecolor(200, 200, 220));
                }
            }
            println!();
        }
...
fn is_direct_dep(package: &str) -> bool {
    ...
    if cargo_toml_dep_names().iter().any(|n| n == package) {
        return true;
    }
    if let Ok(c) = fs::read_to_string("requirements.txt") {
        if c.lines()
            .any(|l| l.trim().to_lowercase().starts_with(&package.to_lowercase()))
        {
            return true;
        }
    }
    for file in &[
        ...
        if let Ok(c) = fs::read_to_string(file) {
            if c.to_lowercase().contains(&package.to_lowercase()) {
                return true;
            }
        }
    }
...
fn trace_why(package: &str) -> Vec<Vec<String>> {
    ...
    if !pkg_deps.is_empty() {
        ...
        for (parent, deps) in &pkg_deps {
            if deps
                .iter()
                .any(|d| d.to_lowercase() == package.to_lowercase())
            {
                if *parent == root {
