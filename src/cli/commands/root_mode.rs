...
pub fn execute_pkg_mode() -> Result<(), InfynonError> {
    ...
    args = if raw_args.len() > 1 && raw_args[1] == "pkg" {
        let filtered: Vec<String> = std::iter::once(raw_args[0].clone()).chain(raw_args[2..].iter().cloned()).collect();
...
fn route_pkg_command(args: PkgArgs, command: PkgCommands) -> Result<(), InfynonError> {
    match command {
        PkgCommands::Scan { output, fix, pkg_file } => {
            let format = output.as_deref().map(|value| match value.to_ascii_lowercase().as_str() {
                "pdf" => OutputFormat::Pdf,
                "both" => OutputFormat::Both,
                _ => OutputFormat::Markdown,
            });
            run_scan(format, fix.map(|value| FixLevel::from_str(&value)), pkg_file.or(args.pkg_file).as_deref(), args.agent);
        }
        PkgCommands::Audit { pkg_file } => features::cmd_audit_deep(pkg_file.or(args.pkg_file).as_deref()),
        PkgCommands::Why { package, pkg_file } => features::cmd_why(&package, pkg_file.or(args.pkg_file).as_deref()),
        PkgCommands::Outdated { pkg_file } => features::cmd_outdated(pkg_file.or(args.pkg_file).as_deref()),
        PkgCommands::Diff { package, v1, v2, ecosystem } => features::cmd_diff(&package, &v1, &v2, ecosystem.as_deref()),
        PkgCommands::Doctor { pkg_file } => features::cmd_doctor(pkg_file.or(args.pkg_file).as_deref()),
        ...
        PkgCommands::Search { query, ecosystem } => features::cmd_search(&query, ecosystem.as_deref()),
        PkgCommands::Fix { auto: _, pkg_file } => features::cmd_fix_auto(pkg_file.or(args.pkg_file).as_deref()),
        PkgCommands::Clean { pkg_file } => features::cmd_clean(pkg_file.or(args.pkg_file).as_deref()),
