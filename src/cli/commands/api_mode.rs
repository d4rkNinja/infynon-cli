...
pub fn execute_api_command(action: ApiCommands) {
    ...
    match action {
        ...
        ApiCommands::Attach { from, to, carry, condition, ai } => attach::cmd_attach(&from, &to, &carry, condition.as_deref(), ai),
        ApiCommands::Detach { from, to } => attach::cmd_detach(&from, &to),
        ApiCommands::Ai { action } => handle_ai_action(action),
        ApiCommands::Import { spec, flow, base_url, prefix, dry_run } => import::cmd_import(&spec, flow.as_deref(), base_url.as_deref(), prefix.as_deref(), dry_run),
...
fn handle_node_action(action: NodeAction) {
    match action {
        ...
        NodeAction::Run { id, base_url, set, prompt } => {
            let url = match base_url.or_else(crate::api::commands::env::env_base_url) { Some(url) => url, None => return Logger::error("BASE_URL is not set. Add it to .infynon/.env or pass --base-url <url>") };
            node::cmd_node_run(&id, &url, &set, prompt);
        }
        NodeAction::Export { id, format, base_url } => node::cmd_node_export(&id, &format, base_url.as_deref()),
        ...
        NodeAction::Prompt { node_id, action } => match action {
            ...
            PromptAction::Add { var, label, secret, default, prompt_type, options } => node::cmd_node_prompt_add(&node_id, &var, &label, secret, default, &prompt_type, options),
...
fn handle_flow_action(action: FlowAction) {
    match action {
        ...
        FlowAction::Merge { flow1, flow2, join_at, name } => flow::cmd_flow_merge(&flow1, &flow2, &join_at, &name),
    }
}
...
fn handle_ai_action(action: AiAction) {
    match action {
        ...
        AiAction::Probe { flow_id, base_url } => ai_cmd::cmd_ai_probe(&flow_id, base_url.as_deref()),
