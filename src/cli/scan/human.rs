...
fn queue_packages(packages: &[scanner::LockedPackage]) -> Vec<(String, String, String)> {
    ...
    let tuples: Vec<_> = packages
        .iter()
        .map(|pkg| {
            pb.set_message(format!("{}@{}", pkg.name, pkg.version));
            pb.inc(1);
            (pkg.name.clone(), pkg.ecosystem.clone(), pkg.version.clone())
        })
        .collect();
...
fn collect_vuln_ids(results: &[Vec<osv::OsvVulnRef>]) -> (Vec<String>, Vec<(String, usize)>) {
    ...
    for (index, vuln_refs) in results.iter().enumerate() {
        for vuln_ref in vuln_refs {
            if seen.insert(vuln_ref.id.clone()) {
                ids.push(vuln_ref.id.clone());
            }
            refs.push((vuln_ref.id.clone(), index));
        }
    }
    (ids, refs)
}
...
fn fetch_detail_map(unique_ids: &[String]) -> HashMap<String, osv::OsvVulnDetail> {
    ...
    for (id, result) in osv::fetch_vuln_details_batch(unique_ids) {
        pb.inc(1);
...
fn build_findings(
    packages: &[scanner::LockedPackage],
    refs: &[(String, usize)],
    details: &HashMap<String, osv::OsvVulnDetail>,
    fix_level: Option<&crate::cli::scan::FixLevel>,
) -> Vec<reporter::ScanFinding> {
    refs.iter()
        .filter_map(|(id, pkg_idx)| {
            ...
            Some(reporter::ScanFinding {
                ...
                vuln: detail.clone(),
                severity,
                fixed_version: osv::best_fixed_version(
                    detail,
...
fn apply_safe_fix_versions(findings: &mut [reporter::ScanFinding]) {
    ...
    for finding in findings.iter() {
        ...
        current
            .entry(key.clone())
            .or_insert_with(|| finding.package.version.clone());
        if let Some(ref fixed) = finding.fixed_version {
            candidates.entry(key).or_default().push(fixed.clone());
        }
    }
    let mut safe_versions = HashMap::new();
    for ((name, eco), versions) in &candidates {
        safe_versions.insert(
            (name.clone(), eco.clone()),
            osv::find_safest_candidate_vs(
                versions,
                name,
                eco,
                current
    ...
    for finding in findings.iter_mut() {
        if let Some(safe) = safe_versions.get(&(
            finding.package.name.clone(),
            finding.package.ecosystem.clone(),
        )) {
            finding.fixed_version = safe.as_ref().map(|(version, _)| version.clone());
        }
    }
}
...
fn apply_latest_suggestions(findings: &mut [reporter::ScanFinding]) {
    ...
    for finding in findings.iter_mut() {
        ...
        let latest = cache.entry(key).or_insert_with(|| {
            let latest = crate::engine::registry::fetch_latest_version(
                &finding.package.name,
                &finding.package.ecosystem,
            )?;
            if osv::version_vuln_count(&finding.package.name, &finding.package.ecosystem, &latest)
                > 0
            {
                None
            } else {
        ...
        if let Some(ref latest) = latest {
            if latest != &finding.package.version {
                finding.suggested_version = Some(latest.clone());
            }
        }
    }
}
...
fn write_reports(
    findings: &[reporter::ScanFinding],
    output: Option<crate::cli::scan::OutputFormat>,
) {
    match output {
        Some(crate::cli::scan::OutputFormat::Markdown)
        | Some(crate::cli::scan::OutputFormat::Both) => {
            match reporter::write_markdown(findings, "infynon-scan-report.md") {
                Ok(_) => Logger::success("Markdown report written → infynon-scan-report.md"),
                Err(err) => Logger::error(&format!("Failed to write markdown: {}", err)),
            }
        }
        _ => {}
    }
    match output {
        Some(crate::cli::scan::OutputFormat::Pdf) | Some(crate::cli::scan::OutputFormat::Both) => {
            match reporter::write_pdf(findings, "infynon-scan-report.pdf") {
                Ok(_) => Logger::success("PDF report written → infynon-scan-report.pdf"),
                Err(err) => Logger::error(&format!("Failed to write PDF: {}", err)),
            }
        }
