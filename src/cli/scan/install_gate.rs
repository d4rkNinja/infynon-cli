...
pub fn check_packages_before_install(
    ...
    ecosystem: &str,
    agent: bool,
) -> Result<(bool, Vec<crate::cli::scan::VulnHit>), String> {
    ...
    let tuples: Vec<(String, String, String)> = names
        .iter()
        .map(|spec| resolve_spec(spec, &osv_eco, agent, &sp))
        .collect::<Result<_, _>>()?;
    if !agent {
    ...
    let results = osv::batch_query(&tuples).map_err(|err| {
        if !agent {
            sp.finish_and_clear();
            Logger::raw_dim(&format!("  CVE check skipped (API error): {}", err));
        }
        format!("vulnerability check failed: {}", err)
    })?;
    if !agent {
        sp.finish_and_clear();
    }
...
fn resolve_spec(
    ...
    if !version.is_empty() {
        return Ok((name, osv_eco.to_string(), version));
    }
    if !agent {
...
fn collect_hits(
    ...
    results: &[Vec<osv::OsvVulnRef>],
    ecosystem: &str,
    agent: bool,
) -> Vec<crate::cli::scan::VulnHit> {
    ...
    for (index, refs) in results.iter().enumerate() {
        ...
        if !agent {
            print_pkg_warning(pkg_name, pkg_ver, refs.len());
        }
        ...
        for vuln_ref in refs.iter().take(5) {
            match osv::fetch_vuln_detail(&vuln_ref.id) {
                Ok(detail) => {
                    ...
                    let fixed = osv::best_fixed_version(&detail, pkg_name, &tuples[index].1);
                    let summary = detail
                        .summary
                        .clone()
                        .unwrap_or_else(|| "No description available".into());
                    if let Some(ref value) = fixed {
                        candidates.push(value.clone());
                    }
                    infos.push((vuln_ref.id.clone(), severity, summary, fixed));
                }
                Err(_) => hits.push(crate::cli::scan::VulnHit {
                    package: pkg_name.clone(),
                    cve_id: vuln_ref.id.clone(),
                    severity: "UNKNOWN",
                    summary: "Could not fetch CVE details".into(),
                    fixed_version: None,
                    upgrade_cmd: None,
                    fix_is_clean: false,
                }),
        ...
        let validated =
            osv::find_safest_candidate_vs(&candidates, pkg_name, &tuples[index].1, pkg_ver);
        for (id, severity, summary, _) in infos {
            let fixed = validated.as_ref().map(|(version, _)| version.clone());
            if !agent {
                println!(
                    "    {}  {}  {}",
                    crate::cli::scan::report::severity_badge(severity),
                    id.truecolor(100, 160, 255),
                    summary
                        .chars()
                        .take(80)
                        .collect::<String>()
            ...
            hits.push(crate::cli::scan::VulnHit {
                package: pkg_name.clone(),
                cve_id: id,
                severity,
                ...
                upgrade_cmd: fixed
                    .as_deref()
                    .map(|value| install_cmd_for_ecosystem(pkg_name, value, ecosystem)),
                fix_is_clean: validated.as_ref().map(|(_, clean)| *clean).unwrap_or(false),
            });
        }
        if !agent {
            print_fix_recommendation(validated.as_ref(), pkg_name, ecosystem, refs.len());
        }
    }
    if !agent {
        println!();
...
fn print_fix_recommendation(
    validated: Option<&(String, bool)>,
    pkg_name: &str,
    ecosystem: &str,
    refs_len: usize,
) {
    match validated {
        Some((version, true)) => println!(
            ...
            install_cmd_for_ecosystem(pkg_name, version, ecosystem).truecolor(100, 140, 100)
        ),
