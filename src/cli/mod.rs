...
fn make_pkg_command(program: &str, args: &[String]) -> std::io::Result<std::process::Command> {
    ...
    ensure_windows_command_line_fits(&resolved_program, args)?;
    #[cfg(windows)]
    if is_windows_command_script(&resolved_program) {
        ...
        c.arg("/D")
            .arg("/S")
            .arg("/C")
            .arg(windows_command_script_invocation(&resolved_program, args));
        return Ok(c);
    }
...
#[cfg(windows)]
fn ensure_windows_command_line_fits(program: &str, args: &[String]) -> std::io::Result<()> {
    let estimated_len = program.len() + args.iter().map(|arg| arg.len() + 3).sum::<usize>();
    if estimated_len > 30_000 {
        Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "package-manager command is too long for Windows process creation; reduce arguments or use the package manager's file-based input",
        ))
    } else {
        Ok(())
    }
...
#[cfg(not(windows))]
fn ensure_windows_command_line_fits(_program: &str, _args: &[String]) -> std::io::Result<()> {
    Ok(())
}
...
#[cfg(windows)]
fn windows_command_script_invocation(program: &str, args: &[String]) -> String {
    std::iter::once("call".to_string())
        .chain(std::iter::once(windows_cmd_quote_arg(program)))
        .chain(args.iter().map(|arg| windows_cmd_quote_arg(arg)))
        .collect::<Vec<_>>()
        .join(" ")
}
...
mod tests {
    use super::windows_command_script_invocation;

    #[test]
    fn windows_command_script_invocation_quotes_metacharacters() {
        let args = vec![
            "left&right".to_string(),
            "%USERPROFILE%\\pkg".to_string(),
            "has space".to_string(),
        ];

        let command = windows_command_script_invocation("C:\\Tools\\npm.cmd", &args);

        assert_eq!(
            command,
            "call \"C:\\Tools\\npm.cmd\" \"left&right\" \"%%USERPROFILE%%\\pkg\" \"has space\""
        );
    }
}
