...
use crate::utils::{
    ensure_dir, ensure_parent_dir, home_infynon_dir, normalize_windows_verbatim_path,
    safe_file_stem,
};
use std::fs;
...
pub fn crash_reports_dir() -> PathBuf {
    home_infynon_dir().join("crash-reports")
}

pub fn crash_report_path(stem: &str) -> PathBuf {
    let stamp = timestamp_now()
        .chars()
        .map(|ch| if ch.is_ascii_alphanumeric() { ch } else { '-' })
        .collect::<String>();
    crash_reports_dir().join(format!("{}-{}.md", safe_file_stem(stem), stamp))
}
...
pub fn ensure_layout() -> Result<(), String> {
    ...
    ensure_dir(&crash_reports_dir()).map_err(|e| e.to_string())?;
...
fn internal_agent_command_templates() -> Result<AgentCommandTemplates, String> {
    ...
    serde_json::from_str::<AgentCommandTemplates>(strip_bom(text)).map_err(|e| e.to_string())
}
...
fn normalize_agent_command(target: &mut String) -> bool {
    ...
    if target.contains("--append-system-prompt-file \"{system_prompt_path}\"") {
        *target = target.replace(
            ...
            "--append-system-prompt-file {quoted_system_prompt_path}",
        );
    }
    if target.contains("--append-system-prompt-file \"{task_start_system_prompt_path}\"") {
        *target = target.replace(
            ...
            "--append-system-prompt-file {quoted_task_start_system_prompt_path}",
        );
    }
    if target.contains("--append-system-prompt {quoted_system_prompt}") {
        *target = target.replace(
            "--append-system-prompt {quoted_system_prompt}",
            "--append-system-prompt-file {quoted_system_prompt_path}",
        );
    }
    if target.contains("--append-system-prompt {quoted_task_start_system_prompt}") {
        *target = target.replace(
    ...
    if target.contains("claude")
        && target.contains("--append-system-prompt-file {quoted_task_start_system_prompt_path}")
        && !target.contains("{quoted_task_start_system_prompt}")
        && !target.contains("{task_start_system_prompt_arg}")
        && !target.contains("{task_start_system_prompt_stdin}")
        && !target.contains("{quoted_prompt}")
    {
        target.push_str(" {task_start_system_prompt_arg}");
    }
    if target.contains("gemini")
        && target.contains("--prompt-interactive {quoted_task_start_system_prompt}")
        && !target.contains("{gemini_task_system_prompt_env}")
    {
        *target = format!("{{gemini_task_system_prompt_env}} {}", target);
    }
    if target.contains("gemini")
        && target.contains("--resume {quoted_session_id}")
        && target.contains("--prompt-interactive {quoted_task_start_system_prompt}")
    {
        *target = target.replace(
            "--prompt-interactive {quoted_task_start_system_prompt}",
            "{quoted_prompt}",
        );
    }
    ...
    if target.contains("model_instructions_file=\"{system_prompt_path}\"") {
        *target = target.replace(
            "model_instructions_file=\"{system_prompt_path}\"",
            "{quoted_codex_system_prompt_config}",
        );
    }
    if target.contains("model_instructions_file=\"{task_start_system_prompt_path}\"") {
        *target = target.replace(
            "model_instructions_file=\"{task_start_system_prompt_path}\"",
...
pub fn load_manifest() -> Result<NinjaManifest, String> {
    ...
    let text = fs::read_to_string(&path).map_err(|e| e.to_string())?;
    let mut manifest =
        serde_yaml::from_str::<NinjaManifest>(strip_bom(&text)).map_err(|e| e.to_string())?;
    if repair_agent_root_default(&mut manifest) | repair_windows_verbatim_paths(&mut manifest) {
        save_manifest(&manifest)?;
    }
    Ok(manifest)
}
...
pub fn write_crash_report(stem: &str, content: &str) -> Result<PathBuf, String> {
    let path = crash_report_path(stem);
    ensure_parent_dir(&path).map_err(|e| e.to_string())?;
    atomic_write_text(&path, content)?;
    mark_hidden_best_effort(&home_infynon_dir());
    Ok(path)
...
pub fn load_workspace(name: &str) -> Result<WorkspaceRecord, String> {
    ...
    let mut workspace =
        serde_json::from_str::<WorkspaceRecord>(strip_bom(&text)).map_err(|e| e.to_string())?;
    if repair_workspace_windows_verbatim_paths(&mut workspace) {
...
pub fn load_task(id: &str) -> Result<TaskRecord, String> {
    ...
    serde_json::from_str::<TaskRecord>(strip_bom(&text)).map_err(|e| e.to_string())
}
...
fn repair_windows_verbatim_paths(manifest: &mut NinjaManifest) -> bool {
    let mut changed = false;
    if let Some(path) = manifest.agent_root_path.as_mut() {
    ...
    for workspace in &mut manifest.workspaces {
        if let Some(path) = workspace.path.as_mut() {
            let normalized = normalize_windows_verbatim_path(path);
            if normalized != *path {
                *path = normalized;
                changed = true;
        ...
        for folder in &mut workspace.folders {
            let normalized = normalize_windows_verbatim_path(&folder.path);
...
fn repair_workspace_windows_verbatim_paths(workspace: &mut WorkspaceRecord) -> bool {
    let mut changed = false;
    if let Some(path) = workspace.path.as_mut() {
        let normalized = normalize_windows_verbatim_path(path);
        if normalized != *path {
...
mod tests {
    ...
    #[test]
    fn migrates_claude_bootstrap_prompt_to_file_argument() {
        let mut command = "claude --append-system-prompt {quoted_system_prompt} --permission-mode bypassPermissions".to_string();

        assert!(normalize_agent_command(&mut command));
        assert_eq!(
            command,
            "claude --append-system-prompt-file {quoted_system_prompt_path} --permission-mode bypassPermissions"
        );
    }

    #[test]
    fn migrates_claude_task_prompt_to_file_argument() {
        let mut command = "claude --append-system-prompt {quoted_task_start_system_prompt} -r {quoted_session_id} {quoted_prompt}".to_string();

        assert!(normalize_agent_command(&mut command));
        assert_eq!(
            command,
            "claude --append-system-prompt-file {quoted_task_start_system_prompt_path} -r {quoted_session_id} {quoted_prompt}"
        );
    }

    #[test]
    fn migrates_claude_task_start_to_include_initial_prompt() {
        let mut command = "claude {model_arg} --append-system-prompt-file {quoted_task_start_system_prompt_path} --permission-mode bypassPermissions --dangerously-skip-permissions".to_string();

        assert!(normalize_agent_command(&mut command));
        assert_eq!(
            command,
            "claude {model_arg} --append-system-prompt-file {quoted_task_start_system_prompt_path} --permission-mode bypassPermissions --dangerously-skip-permissions {task_start_system_prompt_arg}"
        );
    }

    #[test]
    fn keeps_claude_task_start_file_arg_prompt() {
        let mut command = "claude {model_arg} --append-system-prompt-file {quoted_task_start_system_prompt_path} --permission-mode bypassPermissions --dangerously-skip-permissions {task_start_system_prompt_arg}".to_string();
    ...
    #[test]
    fn migrates_gemini_task_start_to_system_prompt_env() {
        let mut command = "gemini {model_arg} --skip-trust --approval-mode=yolo --prompt-interactive {quoted_task_start_system_prompt}".to_string();

        assert!(normalize_agent_command(&mut command));
        assert_eq!(
            command,
            "{gemini_task_system_prompt_env} gemini {model_arg} --skip-trust --approval-mode=yolo --prompt-interactive {quoted_task_start_system_prompt}"
        );
    }

    #[test]
    fn migrates_gemini_resume_to_system_prompt_env_and_resume_prompt() {
        let legacy_prompt_flag = "--prompt-interactive {quoted_task_start_system_prompt}";
        let mut command = format!(
            "gemini {{model_arg}} --skip-trust --approval-mode=yolo --resume {{quoted_session_id}} {legacy_prompt_flag}"
        );

        assert!(normalize_agent_command(&mut command));
        assert_eq!(
            command,
            "{gemini_task_system_prompt_env} gemini {model_arg} --skip-trust --approval-mode=yolo --resume {quoted_session_id} {quoted_prompt}"
        );
    }
}
