...
This started task has two kinds of files:

- Runtime guide: this task-start prompt, generated from `src/ninja/task-start-systemprompt.md` and stored at `{task_start_system_prompt_path}`.
- Durable task record: `{task_json_path}` and `{task_markdown_path}`.

The runtime guide explains how to operate inside the task. It provides lifecycle rules, command usage, closeout requirements, and the active task id. It is not the task assignment.

The durable task record stores the real assignment, status, notes, results, pid, session id, agent, model, workspace, and folder. Use `infynon task show {task_id}` as the source of truth before changing state. Use task commands to update it; do not edit the JSON or markdown directly unless explicitly instructed.

The real task content may be delivered separately as the normal user prompt at launch time, or it may only be available in the durable task record. Always run `infynon task show {task_id}` before planning or executing so you have the current task description, notes, results, status, workspace, folder, and file paths. If any launch prompt and durable task record differ, use the durable task record as the source of truth.

If instructions appear to conflict, follow this order:

1. Higher-priority runtime/system instructions.
2. The task-start runtime guide for lifecycle and command usage.
3. The normal launch prompt for the actual task payload.
4. The durable task record and task notes/results for latest coordination context.
...
- Treat `infynon task show {task_id}` as the user's actual requested work source. A normal launch prompt may provide extra context, but the durable task record is authoritative.
- Prefer GCCD task briefs for delegated child tasks: Time, Goal, Context, Constraints, Done When, and Completion Instructions.
...
Use these commands with the active task id unless creating a child task.

Command intent:

- `infynon task show`: read the full durable task state before acting.
- `infynon task note`: append progress, blockers, decisions, or handoff context.
- `infynon task result`: append findings, outputs, validation summaries, or final material.
- `infynon task update`: change metadata such as pid, session id, model, notes, result, or status.
- `infynon task complete`: mark this task successful and terminal.
- `infynon task fail`: mark this task failed and terminal.
- `infynon task kill`: stop a process and mark the task killed when required.
- `infynon task create`: create a child task for delegated work.
- `infynon task resume`: send follow-up instructions to an existing stored session.

Create a child task with a GCCD assignment:

```bash
infynon task create <child-task-id> --mutate --workspace <workspace> --folder-name <folder> --agent <codex|claude|gemini> --model <model> --prompt "# GCCD Task Brief
...
- On success, run: infynon task complete <child-task-id> --mutate --result \"final result\".
- On failure or blocker, run: infynon task fail <child-task-id> --mutate --reason \"failure reason\".
- Before exiting, run infynon task show <child-task-id> and verify the task is terminal."
```
