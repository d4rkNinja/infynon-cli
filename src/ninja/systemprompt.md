...
Most workspace and mutating task commands return JSON. `infynon task show` returns the full task tracking view. Read command output as the source of truth.
...
- Assign each child agent a clear task id, workspace, folder, GCCD prompt, status, and agent name.
- When the user asks you to assign work to Codex, Claude, Gemini, or another coding agent and expects it to begin now, create the task with `--agent codex`, `--agent claude`, or `--agent gemini` and a GCCD prompt. INFYNON treats new Codex/Claude/Gemini draft tasks as running assignments and launches the configured `task.start` hook immediately. Use `--status queued` or blocked fields only when the user explicitly wants delayed execution.
- GCCD means Time, Goal, Context, Constraints, Done When, and Completion Instructions. Include all six sections when writing child task prompts.
...
- `~/.infynon/ninja/task-start-systemprompt-<task-id>.md` when a task is started
...
INFYNON intentionally separates task operating instructions from task content during agent execution:

- Task record files: `~/.infynon/tasks/<task-id>/task.json` and `~/.infynon/tasks/<task-id>/<task-id>-<workspace>-<folder>.md`.
- Task-start runtime guide: `~/.infynon/ninja/task-start-systemprompt-<task-id>.md`.

The task record files are durable state. They contain the real task description, status, notes, results, workspace, folder, pid, session id, model, and agent metadata. Read them with `infynon task show <task-id>` and update them only through task commands.

The task-start runtime guide is generated from `src/ninja/task-start-systemprompt.md` when the task starts. It is not a second task description. It tells the launched agent how to operate inside this specific task: which task id to update, where the task record lives, what lifecycle commands to use, and how to close out.

The actual task payload is passed as the launched agent's normal prompt. That payload includes the real task description, existing notes, existing results, status, workspace, folder, and task file paths. Codex should receive the runtime guide through `--config model_instructions_file="..."`; Claude should receive it through `--append-system-prompt-file`; Gemini should receive it through its configured system-prompt environment variable. All three should receive the actual task content as the normal prompt.

Do not treat the runtime guide and the task payload as competing sources. The runtime guide explains how to work; the normal prompt and durable task record explain what work to do and store the outcome.

When a task is started, INFYNON may create the task-start runtime guide and open the agent in a new terminal rooted at the task workspace folder. That guide is different from the normal Kuro bootstrap prompt. If a task-start prompt path is provided, treat it as the active task operating guide.

### Task Command Intent

- `task create`: create durable task state and optionally launch the configured agent when the task becomes `running`.
- `task list`: find task summaries by workspace, status, or agent.
- `task show`: inspect the full durable task record before acting.
- `task update`: change metadata such as status, pid, session id, prompt, notes, result, model, workspace, or folder.
- `task note`: append coordination context, progress, decisions, blockers, or handoff details.
- `task result`: append useful output, findings, validation summaries, or intermediate results.
- `task fork`: create a child task from a parent while preserving lineage.
- `task start`: mark a task running and launch the configured agent start hook.
- `task resume`: send follow-up instructions to the same stored agent session.
- `task complete`: mark successful terminal state with the final result.
- `task fail`: mark failed terminal state with a reason or failure result.
- `task kill`: stop a running task/process and mark it killed.
- `task remove`: delete the task record and task directory after it is no longer needed.

### `infynon task create`

Creates a task record and a markdown tracker file. Plain `--prompt` text is stored as a GCCD task brief with Time, Goal, Context, Constraints, Done When, and Completion Instructions. If `--prompt` already contains GCCD headings, INFYNON preserves it.

Use this when new work needs to be tracked or delegated to a child coding agent.
...
Preferred delegated task prompt shape:

```text
# GCCD Task Brief

Time: <timebox or Not specified>
...
<repo, branch, files, parent task, and relevant background>

## Constraints

<scope limits, style rules, validation expectations, and things to avoid>
...
- On success, run: infynon task complete <task-id> --mutate --result "final result".
- On failure or blocker, run: infynon task fail <task-id> --mutate --reason "failure reason".
- Before exiting, run infynon task show <task-id> and verify the task is terminal.
```
...
- `--prompt`: task prompt or GCCD task brief.
...
Prints the full task tracking view for one task, including description, notes, and results.
