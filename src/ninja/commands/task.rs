...
fn task_create_from_parent(
    ...
    let prompt = normalize_task_prompt(prompt);
    ...
    let command_execution = match run_agent_task_action(task.agent.as_deref(), "create", &task) {
        Ok(value) => value,
        Err(err) => {
            mark_task_failed_after_launch_error(&mut manifest, &mut task, &err)?;
            return Err(err);
        }
    };
...
fn task_update(
    ...
    if let Some(prompt) = prompt {
        task.prompt = normalize_task_prompt(Some(prompt));
    }
...
fn task_start(id: &str, pid: Option<u32>, session_id: Option<String>) -> Result<(), String> {
    ...
    if task.status.eq_ignore_ascii_case("running")
        && task.pid.is_some()
        && is_coding_agent(task.agent.as_deref())
        && pid.is_none()
        && session_id.is_none()
    {
        storage::upsert_task_summary(&mut manifest, &task);
        ...
        print_json_pretty(&json!({
            ...
            "task_full_name": task_full_name(&task),
            "task_markdown_path": markdown_path,
            "task_start_system_prompt_path": storage::task_start_system_prompt_path(id),
            "agent_command_template_path": storage::agent_commands_path(),
            "agent_command_execution": {
                ...
                "action": "start",
                "task_id": id,
                "skipped_duplicate_start": true,
                "message": "Task is already running with a recorded PID; INFYNON skipped launching another agent window."
            }
        }));
        return Ok(());
    }
