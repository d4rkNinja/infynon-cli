fn render_project_agent_command_template(
    ...
    let gemini_system_prompt_env = gemini_system_prompt_env(system_prompt_path);
    let quoted_system_prompt = if template.contains("{gemini_system_prompt_env}") {
        shell_quote(&format!(
            "Start an INFYNON coding session. Use the active Gemini system prompt file at {}.",
            system_prompt_path
        ))
    } else {
    ...
    template
        ...
        .replace(
            "{quoted_system_prompt_path}",
            &shell_quote(system_prompt_path),
        )
        .replace(
        ...
        .replace("{quoted_system_prompt}", &quoted_system_prompt)
        .replace("{model}", model.unwrap_or(""))
        .replace("{model_arg}", &model_arg)
        .replace("{gemini_system_prompt_env}", &gemini_system_prompt_env)
}

fn gemini_system_prompt_env(system_prompt_path: &str) -> String {
    if system_prompt_path.trim().is_empty() {
        return String::new();
    }
    if cfg!(windows) {
        format!("$env:GEMINI_SYSTEM_MD = {};", shell_quote(system_prompt_path))
    } else {
        format!("GEMINI_SYSTEM_MD={}", shell_quote(system_prompt_path))
    }
}
