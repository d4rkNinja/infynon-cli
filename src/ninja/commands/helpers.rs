...
fn workspace_folders_from_inputs(
    ...
    match (folder_name, path) {
        (Some(folder_name), Some(path)) => Ok(vec![WorkspaceFolder {
            folder_name,
...
fn normalize_workspace_path(path: &str) -> Result<String, String> {
    ...
    Ok(crate::utils::normalize_windows_verbatim_path(
...
fn normalize_task_prompt(prompt: Option<String>) -> Option<String> {
    let prompt = prompt?;
    let trimmed = prompt.trim();
    if trimmed.is_empty() || is_gccd_prompt(trimmed) {
        return Some(prompt);
    }
    Some(format_gccd_prompt(trimmed))
}

fn is_gccd_prompt(prompt: &str) -> bool {
    let lower = prompt.to_ascii_lowercase();
    lower.contains("# gccd task brief")
        || ((lower.contains("time:") || lower.contains("## time"))
            && lower.contains("## goal")
...
fn format_gccd_prompt(prompt: &str) -> String {
    format!(
        "# GCCD Task Brief\n\n\
Time: Not specified\n\n\
## Goal\n\n\
{prompt}\n\n\
## Context\n\n\
- This task was created from a plain assignment. Treat the Goal as the user's requested outcome.\n\
- Use the assigned workspace and folder as the default working directory.\n\
- Read project files, tests, docs, and local rules before making changes when relevant.\n\n\
## Constraints\n\n\
- Stay scoped to this task and avoid unrelated edits.\n\
- Preserve existing user changes unless explicitly instructed otherwise.\n\
- Add task notes when meaningful progress, blockers, or handoff context changes.\n\
- Validate the result before closing the task, or record why validation was not run.\n\n\
## Done When\n\n\
- The Goal is completed, or a concrete blocker/failure reason is recorded.\n\
- Relevant checks have passed, or skipped checks are explained.\n\
- The final result is persisted with the task lifecycle commands below.\n\n\
...
#[cfg(test)]
mod tests {
    use super::{normalize_workspace_path, workspace_folders_from_inputs};

    #[test]
    fn workspace_folders_from_inputs_canonicalizes_path() {
        let cwd = std::env::current_dir().expect("current dir");
        let cwd_text = cwd.to_string_lossy().to_string();
        let expected = normalize_workspace_path(&cwd_text).expect("normalized cwd");

        let folders =
            workspace_folders_from_inputs(Some("root".to_string()), Some(cwd_text)).unwrap();

        assert_eq!(folders.len(), 1);
        assert_eq!(folders[0].folder_name, "root");
        assert_eq!(folders[0].path, expected);
    }

    #[test]
    fn workspace_folders_from_inputs_requires_name_and_path_together() {
        assert!(workspace_folders_from_inputs(Some("root".to_string()), None).is_err());
        assert!(workspace_folders_from_inputs(None, Some(".".to_string())).is_err());
    }
}
