...
fn go_module_proxy_path(path: &str) -> String {
    path.chars()
        .map(|ch| {
...
#[cfg(test)]
mod tests {
    use super::go_module_proxy_path;

    #[test]
    fn escapes_uppercase_go_module_paths_for_proxy() {
        assert_eq!(
            go_module_proxy_path("github.com/Azure/azure-sdk-for-go"),
            "github.com/!azure/azure-sdk-for-go"
        );
        assert_eq!(
            go_module_proxy_path("github.com/GoogleCloudPlatform/opentelemetry-operations-go"),
            "github.com/!google!cloud!platform/opentelemetry-operations-go"
        );
    }

    #[test]
    fn leaves_lowercase_go_module_paths_unchanged() {
        assert_eq!(
            go_module_proxy_path("golang.org/x/tools"),
            "golang.org/x/tools"
        );
    }
}
